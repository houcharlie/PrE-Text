...
class FLModel(IFLModel):
    ...
    def fl_forward(self, batch) -> BatchMetrics:
        ...
        loss, num_correct, num_examples = self.cross_entropy_eval(output['logits'], tokens)
        loss = loss.cpu()
        del output
        del tokens
        return BatchMetrics(
...
class MetricsReporter(FLMetricsReporter):
    ...
    def compare_metrics(self, eval_metrics, best_metrics):
        ...
        if best_metrics is None:
            return True
        return eval_metrics[self.XENT] < best_metrics[self.XENT]

    def compute_scores(self) -> Dict[str, Any]:
        xent = np.mean(self.losses)
        accuracy = (self.num_correct / (self.num_examples + 1e-6)) * 100

        if np.isnan(xent) or np.isinf(xent):
            raise ValueError("Nan loss")
        return {self.XENT: xent, self.ACCURACY: accuracy.item()}
...
def train(cfg):
    ...
    train_dataset = StackoverflowDataset(
        cfg.data.train_path, cfg.data.num_users
    )
    eval_dataset = StackoverflowDataset(
        cfg.data.eval_path, cfg.data.num_users
    )
    test_dataset = StackoverflowDataset(
    ...
    with open(cfg.result_dict_location, 'w+') as f:
